...
export default function ReceptionistBillingRecords({ sidebarCollapsed = false, toggleSidebar }) {
  const navigate = useNavigate();

  // Get receptionist/clinic info from localStorage for data isolation
  let authUser = {};
  let receptionist = {};
  try {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const allBillsData = Array.isArray(billsRes.data) 
          ? billsRes.data 
      ...
      <div className="flex-grow-1 main-content" style={{ marginLeft: sidebarCollapsed ? 64 : 250 }}>
        ...
        <div className="page-title-bar">
          ...
          <button 
            ...
            onClick={() => navigate("/receptionist/add-bill")}
          >
            <FaPlus /> Add Bill
          </button>
        </div>
          ...
          <div className="search-container">
            <div className="search-input-group">
              ...
              <input
                ...
                placeholder="Search by encounter ID, doctor, clinic, patient, or status..."
                value={searchTerm}
                ...
                <tr className="filter-row">
                  ...
                  <td>
                    <input 
                      className="filter-input" 
                      placeholder="Enc ID" 
                      onChange={(e) => handleFilterChange("encounterId", e.target.value)} 
                    />
                  </td>
                  <td>
                    <input 
                  ...
                  <tr>
                    <td colSpan="12" className="text-center py-5 text-muted">
                      {searchTerm || Object.values(filter).some(v => v)
                        ...
                        : "No billing records found. Click 'Add Bill' to create one."}
                    </td>
                  </tr>
                ) : (
                    ...
                    <tr key={bill._id || i}>
                      ...
                      <td>
                        <div className="action-group">
                          <button 
                            className="btn-icon text-edit" 
                            onClick={() => navigate(`/receptionist/edit-bill/${bill._id}`)}
                            title="Edit Bill"
                          >
                            <FaEdit size={16} />
                          </button>
