...
function ReceptionistProfile() {
  ...
  const receptionistId = authUser?.id;
  ...
  const loadProfile = async () => {
    try {
      ...
      if (!res.ok) {
        ...
        console.error("Receptionist load error:", text);
  ...
  const updateProfile = async (e) => {
    e.preventDefault();
    if (!receptionistId) {
      toast.error("Profile not loaded properly");
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        const text = await res.text();
        console.error("Receptionist update error:", text);
      ...
      // force layout + navbar to re-read localStorage (avatar + name)
  ...
  const letter = form.name?.trim()?.charAt(0)?.toUpperCase() || "R";
